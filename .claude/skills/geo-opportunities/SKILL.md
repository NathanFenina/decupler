...
description: >-
  ...
  mots-clés DataForSEO (volume, difficulté, intention, AI Overview), les données
...
2. **DataForSEO** — `scripts/dataforseo_query.py` : expansion + SERP/AI Overview.
...
- **DataForSEO** : `DATAFORSEO_LOGIN` / `DATAFORSEO_PASSWORD` renseignés dans `.env`.
  Si vides → demander à l'utilisateur, ne pas inventer.
...
```
`--suggestions` ajoute la longue traîne (1 appel API par graine — plus complet,
un peu plus coûteux). L'enlever pour un run rapide/économe.
...
### Étape 3 — SERP / AI Overview (optionnel mais recommandé pour le GEO)
Prendre le top ~30 mots-clés les plus pertinents (par volume × pertinence GEO),
les écrire un par ligne dans `content/opportunities/.cache/top-keywords.txt`, puis :
```bash
...
```
Détecte la présence d'**AI Overview / People Also Ask** = signal GEO pur (sujets
où une réponse IA s'affiche → fort potentiel de citation).
...
**Candidats** = mots-clés DataForSEO **+ requêtes GSC réelles** (chaque requête où
...
⚠️ **Proxy volume** : pour une requête GSC sans donnée DataForSEO, le « volume »
...
requêtes via DataForSEO (search volume) en 2ᵉ passe.

## Garde-fous
- Ne jamais committer `.env` ni le contenu de `.cache/`.
- Ne pas charger l'export XML WordPress (cf. CLAUDE.md).
- Si une brique manque (DataForSEO ou GSC), continuer en dégradé et le dire
