# -*- coding: utf-8 -*-
"""Formulaire de candidature — page « Mini-analyse GEO gratuite » pour
freelances et agences SEO. Même mécanique que form_local.py / form_pme.py :
assistant pas-à-pas en popup, tous les champs obligatoires, pas de Calendly.

Le front-end ici n'est pas une offre commerciale mais une consultation :
on demande ce qu'ils ont déjà payé ces 12 derniers mois (filtre "vrais vs
rêveurs") et ce qui les intéresse (marque blanche / abonnement), en échange
d'une mini-analyse GEO gratuite pour un de leurs clients.

Écrit dans Supabase (projet « LinkedIn App », table `candidatures_seo`,
...
interet_html = "".join(
    f'<label class="rf-check" data-check><input type="checkbox" id="rf-interet-{k}"><span>{lbl}</span></label>'
...
<label class="rf-lbl">Votre structure</label><input type="text" id="rf-entreprise" placeholder="Freelance, ou nom de l'agence">""",
     ["rf-email", "rf-entreprise"]),
    ("Combien de clients gérez-vous actuellement&nbsp;?", "",
     """<select id="rf-clients"><option value="1-3">1 à 3</option><option value="4-10">4 à 10</option><option value="11-25">11 à 25</option><option value="25+">Plus de 25</option></select>""",
     []),
    ("Pour quel client voulez-vous la mini-analyse GEO&nbsp;?", "Le domaine du client à qui elle est destinée — l'analyse sera à votre marque, prête à envoyer.",
     """<input type="url" id="rf-client-domaine" placeholder="https://client-exemple.fr">""",
     ["rf-client-domaine"]),
    ("Une fois la mini-analyse livrée, seriez-vous intéressé par…", "Cochez tout ce qui vous parle.",
     f"""<div class="rf-check-grid">{interet_html}</div>
<label class="rf-lbl">Autre chose&nbsp;?</label><input type="text" id="rf-interet-autre" placeholder="Précisez si besoin">""",
     []),
    ("Sous quel format, et à quel prix mensuel trouveriez-vous ça juste&nbsp;?", "",
     """<label class="rf-lbl">Format souhaité</label><input type="text" id="rf-format" placeholder="ex. autonome, accompagné, clé en main…">
<label class="rf-lbl">Prix mensuel, environ</label><input type="text" id="rf-prix" placeholder="ex. 100€, ou une fourchette">""",
     ["rf-format", "rf-prix"]),
    ("Qu'avez-vous déjà payé ces 12 derniers mois pour des outils ou formations SEO&nbsp;?", "C'est la question la plus utile de tout le formulaire — soyez précis.",
     """<textarea id="rf-deja-paye" placeholder="ex. Semrush, une formation, un freelance sous-traitant…"></textarea>""",
     ["rf-deja-paye"]),
...
<div class="rf-eng"><input type="checkbox" id="rf-engagement"><p>Ok pour un call de 20&nbsp;minutes si votre candidature est retenue&nbsp;? (clôture {cloture})</p></div>""".format(cloture=CLOTURE),
     ["rf-pourquoi"]),
]
...
root.querySelectorAll('[data-next]').forEach(function(btn){{
  btn.addEventListener('click',function(){{
    ...
    if(cur===4){{
      var nChecked=0;
      INTERET_KEYS.forEach(function(k){{if(checked('rf-interet-'+k))nChecked++}});
      var interetBad=(nChecked===0)?(!val('rf-interet-autre')):false;
      if(interetBad){{ok=false;if(!firstBad)firstBad='rf-interet-autre'}}
    }}
    if(!ok){{
...
var CLIENTS={{'1-3':0,'4-10':1,'11-25':2,'25+':3}};
function score(){{
  var s=0;
  s+=DELAI[val('rf-delai')]||0;
  s+=CLIENTS[val('rf-clients')]||0;
  s+=checked('rf-engagement')?2:0;
  s+=val('rf-deja-paye').length>4?1:0;
  return s;
}}
function statut(s){{if(s>=5)return'vert';if(s>=2)return'orange';return'rouge'}}
function interetList(){{
  var out=[];
  INTERET_KEYS.forEach(function(k){{if(checked('rf-interet-'+k))out.push(k)}});
  var autre=val('rf-interet-autre');if(autre)out.push('autre: '+autre);
  return out.join(', ');
}}
var btn=root.querySelector('#rf-submit');
btn.addEventListener('click',function(){{
  var nom=val('rf-nom'),tel=val('rf-tel'),email=val('rf-email'),entreprise=val('rf-entreprise'),pourquoi=val('rf-pourquoi');
  ...
  if(!nom||!tel||!email||!entreprise||!pourquoi||emailBad||engBad){{if(err)err.classList.add('show');return}}
  if(err)err.classList.remove('show');
  var s=score(),st=statut(s);
  var payload={{
    statut:st,score:s,nom:nom,entreprise:entreprise,
    email:email,telephone:tel,
    client_domaine:val('rf-client-domaine'),nb_clients:val('rf-clients'),
    interet:interetList(),format_souhaite:val('rf-format'),prix_mensuel:val('rf-prix'),
    deja_paye:val('rf-deja-paye'),
  ...
  fetch(SUPA_URL+'/rest/v1/candidatures_seo',{{method:'POST',headers:{{
