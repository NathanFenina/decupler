# -*- coding: utf-8 -*-
"""Construit la page « Cartographie GEO gratuite » pour PME —
decupler.com/cartographie-gratuite-pme.

    python3 content/pages-src/build_pme.py

Sort content/articles/cartographie-gratuite-pme.html, prêt pour
wp_publish.py. Formulaire dédié (voir form_pme.py), écrit dans Supabase
(projet « LinkedIn App », table `candidatures_pme`).
"""
import sys, re, json
...
CALENDRIER = [
    ("Vous candidatez", "Deux minutes, quelques questions sur votre activité et vos objectifs."),
    (f"On sélectionne {PLACES} profils", f"On regarde le potentiel réel, pas qui a répondu en premier. Clôture {CLOTURE}."),
...
FAQ = [
    ("C'est vraiment gratuit&nbsp;? Où est le piège&nbsp;?",
     "Il n'y en a pas, et ce n'est pas un cadeau&nbsp;: c'est un échange. On lance une nouvelle offre d'infrastructure GEO et on a besoin de cas documentés, avec des chiffres réels, pour la faire connaître. En échange de la cartographie offerte, on vous demande un retour honnête sur ce qu'elle vous a permis de faire."),
    ("Qu'est-ce qu'une cartographie GEO exactement&nbsp;?",
     "Les prompts exacts où vos concurrents apparaissent dans ChatGPT, Perplexity et les AI&nbsp;Overviews et vous non, les sujets où vous êtes absent, et les pages qui reçoivent du trafic sans jamais convertir."),
    ("Pourquoi un formulaire et pas juste premier arrivé, premier servi&nbsp;?",
     f"Parce qu'on n'a que {PLACES} places pour cette édition, et qu'on préfère les donner aux entreprises où l'impact sera le plus net&nbsp;— pas aux plus rapides à cliquer."),
...
<section class="dcp-hero"><div class="in">
  <div style="max-width:680px">
    ...
    <h1 class="rise">La cartographie GEO de votre marché, <em>offerte</em>, aux PME qui candidatent.</h1>
    <p class="lead rise">On lance une nouvelle offre d'infrastructure GEO. Pour la faire connaître, on offre la cartographie complète de votre marché à {PLACES} entreprises&nbsp;: les prompts où vos concurrents sortent et vous non, vos angles morts, vos pages qui ne convertissent pas — en échange d'un retour honnête sur ce que ça change pour vous.</p>
...
out = str(RACINE / 'content/articles/cartographie-gratuite-pme.html')
