...
attend "ajout du 24/09 → 410" "$(code /casino-en-ligne-suisse/)" "410"
attend "ajout accentué du 24/09 → 410" "$(code '/machines-%C3%A0-sous-libres-sans-inscription/')" "410"
attend "chemin à deux segments → 410" "$(code /detail/11661436)" "410"
...
attend "350 URL dans le sitemap" "$(echo "$SX" | grep -o '<loc>' | wc -l | tr -d ' ')" "350"
attend "lastmod du relevé du 24/09" "$(echo "$SX" | grep -c '<lastmod>2026-09-24</lastmod>')" "350"
...
echo "== 8b. Pages mortes encore indexées (1.3.3)"
attend "/e-e-a-t/ → 301" "$(code /e-e-a-t/)" "301"
attend "cible = /eeat-google/" "$(curl -sS -o /dev/null -w '%{redirect_url}' "$B/e-e-a-t/")" "https://decupler.com/eeat-google/"
